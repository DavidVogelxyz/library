...
The date that is set with `git commit --date` is the commit's "author date".

On a site like GitHub, the date that displays with a commit is known as the "committer date". There is no way to set the "committer date" to "a value other than the current date" when using `git commit`.

However, it *is* possible to change the "committer date" to be the same as the "author date" -- it just requires the user run `git rebase --committer-date-is-author-date -i <HASH_OF_PREVIOUS_COMMIT>`. Note that `<HASH_OF_PREVIOUS_COMMIT>` will refer to the commit before the one for which the user wants to adjust the date.

When `git rebase -i` operates on the commit, it will have its "committer date" changed to the "author date". For more information, check out the section on [interactive rebases](interactive-rebase.md#An-early-note-on-dates-when-rebasing-interactively).
