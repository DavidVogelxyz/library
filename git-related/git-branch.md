...
The command `git switch <NAME>` can also be used -- however, most users use `git checkout` in place of `git switch`. This is due to `git checkout`'s versatility -- it can be used to checkout commits and tags, in addition to branches. However, for future reference, a new branch can be commit with `git switch` by running the command `git switch -c <NAME>`
...
Note the following: `git checkout -b` will attempt to create a new branch, and then switch to it. If a branch already exists with the provided name, the command will fail with the error: `fatal: a branch named '<NAME>' already exists`. A switch similar to `-b` exists: `-B`. In a situation where branch "<NAME>" already exists, `-B` will succeed -- Git will reset ("delete") the branch and check it out. Therefore, it's highly advisable to use only `-b`, or to use `git branch <NAME>` to create the new branch, and then check it out.
...
Another method is to run `git status` -- the first line of the output will say `on branch <NAME>`.
...
This will force the deletion of the branch, similar to how `git checkout -B <NAME>` forces the creation of a branch (even if it already exists). Using `git branch -d <NAME>` works as well, but only if the branch has been fully merged into the upstream branch, or it will fail with error: `error: the branch '<NAME>' is not fully merged`.
...
Just like with other commands, `-M` will also perform the same action, but will force the name change. In some cases, `-m` will fail -- for example, if there's already a branch named `<NAME>`. When using `-M`, Git will reset ("delete") the branch with the name `<NAME>` and rename the current branch to `<NAME>`. Use `-M` with caution, as `-m` will warn the user that `a branch named '<NAME>' already exists`, while `-M` will not.
