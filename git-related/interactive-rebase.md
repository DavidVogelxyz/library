...
As discussed in the [section on "git commit"](git-commit.md#A-note-on-setting-the-date), Git keeps track of two separate date values: "committer date" and "author date". When rebasing interactively, the committer date does not change; however, the author date does change.
